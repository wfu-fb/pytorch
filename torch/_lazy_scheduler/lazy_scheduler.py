...
def remove_generated_wait_if_explicit_wait_exists(gm):
  ...
  for i in range(len(node_list)):
    ...
    if (
      ...
      assert "nn_module_method" in node_list[i+1].meta  # explicit wait_tensor should always have nn_module_method metadata
      node_list[i+1].args = (n.args[0],)
      n.replace_all_uses_with(node_list[i+1])
